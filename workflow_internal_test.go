...
func Test_runOnce(t *testing.T) {
	...
	t.Run("Retry awaitRole error that is not context.Canceled", func(t *testing.T) {
		...
		require.Nil(t, err)
		require.True(t, strings.Contains(buf.String(), `"msg":"test error","meta":{"retrying_in":"1m0s","role":"role-1"`))
	})
	...
	t.Run("Retry process error that is not context.Canceled", func(t *testing.T) {
		...
		require.Nil(t, err)
		require.True(t, strings.Contains(buf.String(), `"msg":"test error","meta":{"retrying_in":"1ms","role":"role-1"`))
	})
